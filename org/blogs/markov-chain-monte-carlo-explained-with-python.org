...
* Introduction
The point I want to make in this article is that, /with real world, noisy data, and a simulation of the damped harmonic oscillator, we can find a probability distribution of possible parameter values using an MCMC method/.
...
Finally, we have a data set, $y_{sample}$ that tells us the displacement of our mass at $n$ time steps
...
First we need to initiate the spring constant ~k~, and damping constant ~c~, that we will eventually solve for.
...
y_true_sampled = y_true[time_sample]
...
This will predict the next /half time step value/ using the previous values. Then use the midpoint $y$ to update $y_dot$ and the midpoint $y_dot$ to update $y$.
...
Here are model is simple, but in a real world MCMC problem our model could be a complicated PDE solver.
